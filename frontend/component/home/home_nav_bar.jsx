  ...
  handleKeyDown(e) {
    ...
    if (this.props.search.length > 0) {
      if (e.keyCode === 38 && cursor > 0) {
        ...
        const targetURL = document.getElementsByClassName(
            'Search-Bar-Result-List-Items-Active')[0].id;
  ...
  render() {
    ...
    if ((this.props.search.length > 0 ) ||
      ...
      results = this.props.search.map( (result, idx) => {
        return (
          <li key={result.id}
            className={this.state.cursor === idx ?
                'Search-Bar-Result-List-Items-Active' :
                'Search-Bar-Result-List-Items'}
            id={result.id}
          ...
          >
            <Link to={`/show/${result.id}`}>
              <div className='Search-Bar-Result-List-Items-Symnbol'>
                {result.ticker_symbol}
              </div>
              <div className='Search-Bar-Result-List-Items-Name'>
                {result.name}
              </div>
            </Link>
          </li>
      ...
      <div className="homepage-nav-bar">
        ...
        <div>
          <ul className="homepage-nav-list">
            ...
            <div className='homepage-nav-mode'
