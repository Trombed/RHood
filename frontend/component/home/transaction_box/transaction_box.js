...
class TransactionBox extends React.Component {
  ...
  changeBuyShare(e) {
    const returnValue = Number(Number(e.target.value) *
      this.props.sharesPrice).toLocaleString('en', {
  ...
  handleBuy() {
    ...
    } else if ((this.state.shareToBuy * this.props.sharesPrice) >
        this.state.funds) {
      return this.setState({
  ...
  handleSell() {
    if (this.state.currentlyOwned < this.state.shareToBuy ||
          this.state.shareToBuy === 0) {
      return this.setState({
    ...
    return (
      <div className="Transaction-Box-Container">
        ...
        <div className="Transaction-Box-Buying-Power">
          {this.state.funds
              .toLocaleString('en', {
                style: 'currency', currency: 'USD'})} Buying Power
